...
/**
 * Desc:可以多行的RadioGroup
 * 参考链接：http://www.cnblogs.com/a284628487/p/4494669.html
 *
 * 新加设计理念：
 * 1、固定每个item宽度，根据屏幕宽度以及一行个数计算出一个item的宽度
 * 2、新加参数childNumLine 控制一行item个数
 */
...
public class MultiLineRadioGroup extends ViewGroup implements View.OnClickListener {
    ...
    public MultiLineRadioGroup(Context context, AttributeSet attrs,
        ...
        gravity = arr.getInt(R.styleable.MultiLineRadioGroup_child_gravity, LEFT);
        childNumLine = arr.getInt(R.styleable.MultiLineRadioGroup_mlrg_line_num, childNumLine);
        if (childResId == 0) {
...
//                    flagY++;
    ...
    private CheckBox getChild() {
        ...
        LayoutParams params = cb.getLayoutParams();
        params.width = DimensionTool.getScreenWidth(getContext()) / childNumLine - childMarginHorizontal * 2;
